...
public:
    ...
    vector<double> RUN_NETWORK_PROCESS_LAST_FOR_BP;//用于存储最后一层神经网络运算过程数据为了反向传播算法
    vector<double> RUN_NETWORK_PROCESS_LAST;//用于存储最后一层神经网络运算过程数据
    vector<vector<vector<vector<vector<double>>>>> NETWORK_WEIGHT;//除最后一层外的所有权重
    vector<vector<vector<double>>> LAST_NETWORK_WEIGHT;//最后一层的权重
    ...
    double Gradient_Descent(vector<vector<vector<double>>> IN_IMAGES_DATA, vector<vector<double>> target_) {//梯度下降算法,输入目标值,输入和反向传播算法一样,输出损失函数的输出，此算法很慢，不建议使用,而且我可能写错了
